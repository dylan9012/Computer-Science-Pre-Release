...
def ValidJump(Board, PlayersPieces, Piece, NewRow, NewColumn):
  ...
  if NewRow in range(BOARD_SIZE) and NewColumn in range(BOARD_SIZE):  
    if Board[NewRow][NewColumn] == SPACE: 
      ...
      #removed MiddlePiecePlayer == OppositePiecePlayer and
...
def ListPossibleMoves(Board, PlayersPieces, NextPlayer, ListOfMoves):
  ...
  for i in range(1, NUMBER_OF_PIECES + 1):
    ...
    if ValidJump(Board, PlayersPieces, Piece, JumpRow, JumpRightColumn):
      JumpedOver = Board[NewRow][RightColumn] #new for working out which piece is jumped
      if JumpedOver[0] != Piece[0]: #new to see if they are taking a piece
        take.append([Piece, JumpRow, JumpRightColumn]) #new for adding takes to list
      else: #new if jumping own piece
        jump.append([Piece, JumpRow, JumpRightColumn]) #new for adding jumps to list


  if len(take) > 0: #new check to see if any takes
  ...
  if PieceIndex > 0:
    ...
    if Jumping:
      ...
      if Piece[0] != MiddlePiece[0]: #new if pieces belong to different people
        Board[MiddlePieceRow][MiddlePieceColumn] = ' ' #new set piece to empty ######note this is not '' it is ' '
