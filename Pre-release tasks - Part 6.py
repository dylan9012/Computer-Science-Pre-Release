...
def ListPossibleMoves(Board, PlayersPieces, NextPlayer, ListOfMoves):
  ...
  for i in range(1, NUMBER_OF_PIECES + 1):
    ...
    if ValidMove(Board, NewRow, LeftColumn) and checkDame(Board, NextPlayer, CurrentRow, NewRow, LeftColumn):
    ...
    if ValidMove(Board, NewRow, RightColumn) and checkDame(Board, NextPlayer, CurrentRow, NewRow, RightColumn):
...
def checkDame(Board, NextPlayer, CurrentRow, NewRow, LeftColumn):
  if CurrentRow == 1 or CurrentRow == 6:
    NewRow, NewColumn = MoveDame(Board, NextPlayer, NewRow, LeftColumn)
    if NewRow == CurrentRow - 1 or NewRow == CurrentRow +1 :
      return False
    else:
      return True
  else:
