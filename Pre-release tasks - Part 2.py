...
def ListPossibleMoves(Board, PlayersPieces, NextPlayer, ListOfMoves):
    ...
    for i in range(1, NUMBER_OF_PIECES + 1):
        ...
        if PlayersPieces[i][DAME] == 1:
            ...
            if ValidMove(Board, NewRow2, RightColumn):
                print(Piece, ' can move to ', NewRow2, ' , ', RightColumn)
                NumberOfMoves += 1
                ListOfMoves[NumberOfMoves].Piece = Piece
                ListOfMoves[NumberOfMoves].NewRow = NewRow2
                ListOfMoves[NumberOfMoves].NewColumn = RightColumn
                ListOfMoves[NumberOfMoves].CanJump = False
            JumpRow2 = CurrentRow + Direction + Direction
            JumpLeftColumn2 = CurrentColumn - 2
            JumpRightColumn2 = CurrentColumn + 2
            if ValidJump(Board, PlayersPieces, Piece, JumpRow2, JumpLeftColumn2):
                print(Piece, ' can jump to ', JumpRow2, ' , ', JumpLeftColumn)
                NumberOfMoves += 1
                ListOfMoves[NumberOfMoves].Piece = Piece
                ListOfMoves[NumberOfMoves].NewRow = JumpRow2
                ListOfMoves[NumberOfMoves].NewColumn = JumpLeftColumn2
                ListOfMoves[NumberOfMoves].CanJumpFileHandle
            if ValidJump(Board, PlayersPieces, Piece, JumpRow2, JumpRightColumn2):
                print(Piece, ' can jump to ', JumpRow2, ' , ', JumpRightColumn)
                NumberOfMoves += 1
                ListOfMoves[NumberOfMoves].Piece = Piece
                ListOfMoves[NumberOfMoves].NewRow = JumpRow2
...
def Game():
    ...
    filename = input("Please enter the filename you wish to save this game as: ")
    if filename == '':
        filename = "PlayerFile.txt"
    if not FileFound:
        GameEnd = True
    while not GameEnd:
        PrintPlayerPieces(A, B)
